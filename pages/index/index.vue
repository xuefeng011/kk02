<template>
	<view>
		...
		<view class="cu-chat bg-white text-black padding-sm text-center solid-bottom">
			...
			<view class="cu-list menu sm-border">
				<view class="cu-item arrow" v-for="item in detail.details" :key="item._id">
					<view class="content flex row " @click="handleEditAccount(item)">
						...
						<text class="text-price fr text-gray" v-if="!account[detail.type].isfu" style="align-self: center;">{{ item.money }}</text>
...
export default {
	...
	components: {},
	onLaunch: function() {
		console.error('index onLaunch', this.hasLogin, this.userInfo);
	},
	onShow: function() {
		console.error('index Show', this.hasLogin, this.userInfo);
