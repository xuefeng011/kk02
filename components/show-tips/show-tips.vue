<template name="redflower-showTips">
	<view>
		<view class="tipsWrap" @tap="closeTips()" v-if="showTips"  :style="[{top:CustomBar + 'px'}]">
			<view class="arrow"></view>
			<view class="tipsContent bg-yellow text-white">
				<text>添加到我的小程序，下次使用更便捷</text>
				<text class="closeIcon" >X</text>
			</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		mounted() {
			// console.log("x11111111111111111111111111111111xx")
			const _that = this;
			//获取本地缓存
			let cache = uni.getStorageSync('isFirst');
			 const tipshow = uni.getStorageSync('tipshow');
			// cache=false;
			if(!tipshow){
